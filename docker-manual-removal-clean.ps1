    ...
    if ($dockerService) {
        if ($dockerService.Status -eq "Running") {
            ...
            if ($confirm -eq 'y' -or $confirm -eq 'Y') {
                ...
                Write-Host "   Skipped stopping Docker service" -ForegroundColor Yellow
            }
        } else {
            Write-Host "   Docker service already stopped" -ForegroundColor Blue
        }
    } else {
        Write-Host "   Docker service not found" -ForegroundColor Blue
    }
} catch {
    ...
    if ($confirm -eq 'y' -or $confirm -eq 'Y') {
        try {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "   Docker service deletion result: $result" -ForegroundColor Blue
            }
        } catch {
            Write-Host "   Error deleting service: $($_.Exception.Message)" -ForegroundColor Yellow
        }
    } else {
        Write-Host "   Skipped deleting Docker service" -ForegroundColor Yellow
    }
} else {
    Write-Host "   Docker service not found" -ForegroundColor Blue
}
...
if ($foundInstallations.Count -gt 0) {
    ...
    if ($confirm -eq 'y' -or $confirm -eq 'Y') {
        ...
        Write-Host "   Skipped removing Docker installations" -ForegroundColor Yellow
    }
} else {
    Write-Host "   No Docker installations found" -ForegroundColor Blue
}
...
if (Test-Path $dockerDataPath) {
    ...
    try {
        $itemCount = (Get-ChildItem -Path $dockerDataPath -Recurse | Measure-Object).Count
        Write-Host "   Contains $itemCount items (containers, images, volumes, etc.)" -ForegroundColor Yellow
    } catch {
    ...
    if ($confirm -eq 'y' -or $confirm -eq 'Y') {
        ...
        Write-Host "   Skipped removing Docker data directory" -ForegroundColor Yellow
    }
} else {
    Write-Host "   Docker data directory not found" -ForegroundColor Blue
}
...
if ($dockerPathEntries) {
    ...
    if ($confirm -eq 'y' -or $confirm -eq 'Y') {
        ...
        Write-Host "   Skipped removing Docker from PATH" -ForegroundColor Yellow
    }
} else {
    Write-Host "   No Docker entries found in PATH" -ForegroundColor Blue
}
    ...
    if ($confirm -eq 'y' -or $confirm -eq 'Y') {
        foreach ($regPath in $foundRegistryEntries) {
            ...
            try {
                ...
                Write-Host "   Could not remove registry entry: $regPath" -ForegroundColor Yellow
            }
        }
    } else {
        Write-Host "   Skipped removing registry entries" -ForegroundColor Yellow
    }
} else {
    Write-Host "   No Docker registry entries found" -ForegroundColor Blue
}
...
if ($dockerProcesses) {
    Write-Host "   Found remaining Docker processes:" -ForegroundColor Yellow
...
if ($dockerServices) {
    Write-Host "   Found remaining Docker services:" -ForegroundColor Yellow
...
try {
    ...
    if ($dockerTest) {
        Write-Host "   docker.exe still available at: $($dockerTest.Source)" -ForegroundColor Yellow
    } else {
